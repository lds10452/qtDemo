...
#include <QTextEdit>
#include <QMessageBox>
ImgEditMainWindow::ImgEditMainWindow(QWidget *parent)
    : QMainWindow(parent)
{
    ...
    createMenus();
    createToolBars();
    if(img.load(":/res/images/icon.png"))
    {
       //在imageLabel对象中放置图像
...
void ImgEditMainWindow::createActions()
{
    //“打开”动作
    openFileAction =new QAction(QIcon(picPath+"open.png"),tr("打开"),this);//(a)
    openFileAction->setShortcut(tr("Ctrl+O"));                    //(b)
    openFileAction->setStatusTip(tr("打开一个文件"));               //(c)
    connect(openFileAction,SIGNAL(triggered()),this,SLOT(ShowOpenFile()));
    //“新建”动作
    NewFileAction =new QAction(QIcon(picPath+"new.png"),tr("新建"),this);
    ...
    leftAction =new QAction(QIcon(picPath+"left.png"),"左对齐",actGrp);
    leftAction->setCheckable(true);
    rightAction =new QAction(QIcon(picPath+"right.png"),"右对齐",actGrp);
    rightAction->setCheckable(true);
    centerAction =new QAction(QIcon(picPath+"center.png"),"居中",actGrp);
    centerAction->setCheckable(true);
    justifyAction =new QAction(QIcon(picPath+"justify.png"),"两端对齐",actGrp);
    justifyAction->setCheckable(true);
    connect(actGrp,SIGNAL(triggered(QAction*)),this,SLOT(ShowAlignment (QAction*)));
    //实现撤销和恢复的动作（Action）
    //撤销和恢复
    undoAction =new QAction(QIcon(picPath+"undo.png"),"撤销",this);
    connect(undoAction,SIGNAL(triggered()),showWidget->text,SLOT (undo()));
    redoAction =new QAction(QIcon(picPath+"redo.png"),"重做",this);
    connect(redoAction,SIGNAL(triggered()),showWidget->text,SLOT (redo()));
}

void ImgEditMainWindow::createMenus()
{
    //文件菜单
...
void ImgEditMainWindow::createToolBars()
{
    //文件工具条
    ...
    addToolBar(Qt::LeftToolBarArea,zoomTool);
    //旋转工具条
    rotateTool =new QToolBar;
    rotateTool->addAction(rotate90Action);
    rotateTool->addAction(rotate180Action);
    rotateTool->addAction(rotate270Action);
    addToolBar(Qt::RightToolBarArea,rotateTool);
    //撤销和重做工具条
    ...
    addToolBar(Qt::BottomToolBarArea,listToolBar);
}

void ImgEditMainWindow::creatOther()
{
    //在工具栏上嵌入控件
    //设置字体
    ...
    boldBtn->setIcon(QIcon(picPath+"bold.png"));
    boldBtn->setCheckable(true);
    italicBtn =new QToolButton;
    italicBtn->setIcon(QIcon(picPath+"italic.png"));
    italicBtn->setCheckable(true);
    underlineBtn =new QToolButton;
    underlineBtn->setIcon(QIcon(picPath+"underline.png"));
    underlineBtn->setCheckable(true);
    colorBtn =new QToolButton;
    colorBtn->setIcon(QIcon(picPath+"color.png"));
...
void ImgEditMainWindow::ShowNewFile()
{
    ImgEditMainWindow *newImgProcessor =new ImgEditMainWindow;
    newImgProcessor->show();
}

void ImgEditMainWindow::ShowOpenFile()
{
...
void ImgEditMainWindow::loadFile(QString filename)
{
    printf("file name:%s\n",filename.data());
    QFile file(filename);
...
void ImgEditMainWindow::ShowPrintText()
{
...
void ImgEditMainWindow::ShowPrintImage()
{
    QPrinter printer;							//新建一个QPrinter对象
    QPrintDialog printDialog(&printer,this);	//(a)
    if(printDialog.exec())						//(b)
...
void ImgEditMainWindow::ShowZoomIn()
{
    if(img.isNull())				//有效性判断
        return;
...
void ImgEditMainWindow::ShowZoomOut()
{
    if(img.isNull())
        return;
    QMatrix matrix;
...
void ImgEditMainWindow::ShowRotate90()
{
    if(img.isNull())
        return;
    QMatrix matrix;
...
void ImgEditMainWindow::ShowRotate180()
{
    if(img.isNull())
        return;
    QMatrix matrix;
...
void ImgEditMainWindow::ShowRotate270()
{
    if(img.isNull())
        return;
    QMatrix matrix;
...
void ImgEditMainWindow::ShowMirrorVertical()
{
    if(img.isNull())
        return;
...
void ImgEditMainWindow::ShowMirrorHorizontal()
{
    if(img.isNull())
        return;
...
void ImgEditMainWindow::ShowFontComboBox(QString comboStr)	//设置字体
{
...
void ImgEditMainWindow::mergeFormat(QTextCharFormat format)
{
...
void ImgEditMainWindow::ShowSizeSpinBox(QString spinValue)	//设置字号
{
   QTextCharFormat fmt;
...
void ImgEditMainWindow::ShowBoldBtn()                        //设置文字显示加粗
{
    QTextCharFormat fmt;
...
void ImgEditMainWindow::ShowItalicBtn()                      //设置文字显示斜体
{
    QTextCharFormat fmt;
...
void ImgEditMainWindow::ShowUnderlineBtn()                   //设置文字加下画线
{
    QTextCharFormat fmt;
...
void ImgEditMainWindow::ShowColorBtn()						//设置文字颜色
{
...
void ImgEditMainWindow::ShowCurrentFormatChanged(const QTextCharFormat &fmt)
{
...
void ImgEditMainWindow::ShowAlignment(QAction *act)
{
    if(act==leftAction)
...
void ImgEditMainWindow::ShowCursorPositionChanged()
{
...
void ImgEditMainWindow::ShowAboutQt()
{
    QMessageBox::aboutQt(this,"qt");
}

void ImgEditMainWindow::ShowList(int index)
{
...
ImgEditMainWindow::~ImgEditMainWindow()
{

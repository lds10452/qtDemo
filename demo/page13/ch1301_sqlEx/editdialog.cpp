...
#include <QPushButton>
int uniqueCarId=1;
int uniqueFactoryId=1;
EditDialog::EditDialog(QSqlRelationalTableModel *cars, QSqlTableModel *factory,QDomDocument details, QFile *output)
{
    carModel=cars;
...
void EditDialog::submit()
{
    ...
    else
    {
        ...
        QStringList attribs=attribEditor->text().split(";",QString::SkipEmptyParts);
...
void EditDialog::addAttribs(int carId, QStringList attribs)
{
    ...
    for (int i=0;i<attribs.count();i++) {
        ...
        attribNode.setAttribute("number",num);
